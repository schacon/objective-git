...
- (void) sendPackData
{
	...
	while ( (current = [e nextObject]) ) {
		obj = [gitRepo objectFromSha:current];
		size = [obj size];
		...
		data = [[obj contentsData] compressedData];
...
{
	NSString *parentSha;
	GITCommit *commit = [gitRepo commitFromSha:shaValue];
...
- (void) unpackDeltified:(int)type size:(int)size {
	if(type == OBJ_REF_DELTA) {
		...
		if([gitRepo hasObject:sha1]) {
			GITObject *object;
			object = [gitRepo objectFromSha:sha1];
...
- (NSData *) patchDelta:(NSData *)deltaData withObject:(GITObject *)gitObject
{
	...
	NSData *source = [gitObject contentsData];
